...
<%@ page import = "java.util.List, com.bit.model.dto.ReviewDto" %>
<!DOCTYPE html>
<html>
<head>
...
<% List<ReviewDto> list = (List)request.getAttribute("review"); %>
          ...
          <tbody>
			<%
				for(int i=0; i<list.size(); i++){
					ReviewDto bean = list.get(i);
			%>
          
			<tr>
				<td><a href="Mreviewdetail.bit?idx=<%=bean.getNum() %>"><%=bean.getNum() %></a></td>
				<td><a href="Mreviewdetail.bit?idx=<%=bean.getNum() %>"><%=bean.getSub() %></a></td>
				<td><a href="Mreviewdetail.bit?idx=<%=bean.getNum() %>"><%=bean.getWday() %></a></td>
            </tr>
			<%} %>
		 <!--thead th 속성 값 적용-->  	
		 </tbody>
